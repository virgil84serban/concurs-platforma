...
export default function ScoresPage() {
  ...
  async function loadScores(judgeId: string) {
    ...
    const rows = (data as ExistingScore[]) || []
    setScores(rows)
  ...
  useEffect(() => {
    if (selectedJudge) {
      void loadCriteria(selectedJudge)
      void loadScores(selectedJudge)
    }
  }, [selectedJudge])
  ...
  function getCriteriaForPerformance(performance: Performance) {
    const type = performance.categories?.[0]?.formation_type?.toLowerCase()
